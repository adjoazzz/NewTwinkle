...
export default function PreviewScreen({ navigation, route }) {
  ...
  const [selectedView, setSelectedView] = useState('lock'); // 'lock' or 'home'

  // For demo purposes, using first two images or placeholders
  const lockScreenImage = images[0] || null;
  const homeScreenImage = images[1] || null;
  ...
  const handleContinue = () => {
    // Save the collection and navigate to overview
...
const styles = StyleSheet.create({
  ...
  phonePreview: {
    width: '100%',
    aspectRatio: 0.47, // iPhone-like aspect ratio
